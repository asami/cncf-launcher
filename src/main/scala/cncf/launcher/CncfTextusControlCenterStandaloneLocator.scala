...
object CncfTextusControlCenterStandaloneLocator {
  ...
  private def _resolve(paths: LauncherPaths): Option[CncfTextusControlCenterStandaloneRegistration] = {
    ...
    if (!Files.isRegularFile(locator) || Files.isSymbolicLink(locator) || !locator.startsWith(root)) {
      ...
      } yield CncfTextusControlCenterStandaloneRegistration(
        CncfTextusControlCenterRegistrationConfig(
          endpoint = endpoint,
          tokenEnv = "",
          timeout = timeout,
