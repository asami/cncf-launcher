...
final case class LauncherPaths(
  ...
  val globalConfig: Path = cncfHome.resolve("config.yaml")
  val projectConfig: Path = cwd.resolve(".cncf").resolve("config.yaml")
  ...
  val localRepository: Path = cncfHome.resolve("repository")
